...
1. Tras cada sync exitoso de una foto, cuenta `trainingPhotoRepo.countSyncedSinceByCategoryChannel(categoryId, canal, model.trained_at)` para los 3 canales — cuenta fotos con `cv_sync_status='SYNCED'`, y si `trained_at` ya tiene valor, solo las sincronizadas DESPUÉS de esa fecha (implementa "primera vez: 15" y "+15 acumuladas desde el último entrenamiento" de la spec 4.3 con un solo mecanismo).
2. Al llegar cualquier canal a `SHELF_TRAINING_THRESHOLD` (default 15): marca `IMAGES_UPLOADED` y llama `startTraining()` fire-and-forget (no bloquea el sync). `startTraining()` entrena el **proyecto CV completo** (`trainProject()` no es por-tag), cumpliendo "20 OMT + 15 DTT → entrena con las 35" de la spec.
...
4. Al completar (`Completed` en CV), `modelTrainingService.handleTrainingCompleted()` guarda métricas (solo monitoreo, sin gate), llama `aiModelRepo.markTrained()` (avanza `trained_at` SIEMPRE, incluso si `saveMetrics()` falla por la migración 006 no aplicada — necesario para que el conteo del punto 1 no quede roto), marca `status='TRAINED'`, y llama a `publishAndActivate()`:
...
⚠ **Dos bloqueantes de Azure para que la publicación funcione de punta a punta (NINGUNO de código)**:
1. `prediction_resource_id` se guarda `NULL` si el Resource ID de ARM real excede `VARCHAR(100)` (ver `aiInfrastructureService.js`, BUG 2026-07-19).
2. Aun con un id sintácticamente válido, el Resource ID actual de `.env` devolvió `BadRequestInvalidPublishTarget` contra Custom Vision real (probado 2026-08-03) — probablemente no es el recurso de predicción correcto. Se gestiona por fuera de este repo.

Probado (sin tocar la BD real de `sqldb-rscope-prod` — stubs vía `require.cache`, mismo patrón que `scripts/test-user-role-gates.js`): dispatch al llegar al umbral, no-dispatch por debajo, skip mientras `TRAINING`, no-bloqueo por `PUBLISHED`/`READY` viejo, y los 3 caminos de `publishAndActivate` (OK mockeado, rechazo de red simulado, `predictionResourceId=null` con la guardia REAL de `publishIteration`). Ver `docs/DIAGNOSTICO-spec-v1.4-vs-codigo.md` para el diagnóstico que motivó este cableado.
...
⚠ **Migración 006 verificada como NO aplicada en la BD real** (confirmado con `INFORMATION_SCHEMA`, última vez 2026-07-25) — pese a que `aiModelRepo.js` ya asume que las columnas `precision_score`/`recall_score`/`mean_ap`/`metrics_json` existen. `aiModelRepo.saveMetrics()` lanza `"Invalid column name"` hasta que se corra esa migración manualmente. `handleTrainingCompleted()` envuelve ese llamado en try/catch para que un fallo de guardado de métricas nunca impida pasar a `TRAINED` ni bloquee la publicación — las métricas quedan logueadas mientras tanto, y `aiModelRepo.markTrained()` avanza `trained_at` independientemente de este try/catch (ver más arriba). No confiar en que la migración 006 ya está aplicada solo porque el código la asume.
...
- **`modelPrecision.{avgPct,deltaPct,series}`** — depends on migration 006 (`precision_score` on `RETSC_AI_DETECTION_MODELS`), which is **still not applied** (see the training section below) and was explicitly **not** applied as part of this change — coordinate separately before running it. Until then, `getModelPrecisionAvg()` catches `Invalid column name` and the endpoint always returns the full `{avgPct: null, deltaPct: null, series: []}` shape rather than omitting the key, so the frontend can tell "no data yet" apart from "not implemented." Per-enterprise scope joins through `RETSC_OP_ENTERPRISE_CATEGORIES`'s resolved smart categories (`RETSC_AI_DETECTION_MODELS` has no `enterprise_id` of its own — models are per DTC category, shared across enterprises). Also inherits the known `is_active` non-uniqueness bug (`docs/BUG-is_active-no-unico.md`) — not fixed here, may skew the average.
...
**Issue #35 (tags por proyecto) y 8.4 (publicación) — COMPLETADOS 2026-08-03**: `listTags()`, `createTag()`, `ensureTag()` (crea/reusa por nombre, reemplaza las env vars `CV_TAG_*`) y `publishIteration()` (con guardia si `predictionResourceId` es null/inválido) ya existen, probados contra CV real y cableados al flujo automático (ver "Entrenamiento de modelos y publicación automática" arriba). Lo único pendiente ahora es la config de Azure, no el código:
- `prediction_resource_id` se guarda `NULL` si el Resource ID de ARM excede `VARCHAR(100)` (BUG 2026-07-19, `aiInfrastructureService.js`).
- Aun con un id válido, el Resource ID actual de `.env` devolvió `BadRequestInvalidPublishTarget` contra CV real (probado 2026-08-03) — no es el recurso de predicción correcto; se gestiona por fuera de este repo.
- Correr la migración `006_add_metrics_to_detection_models.sql` — verificada como no aplicada en la BD real (ver sección de entrenamiento); sin ella `aiModelRepo.saveMetrics()` falla (no bloquea el flujo — `markTrained()` avanza `trained_at` igual).
