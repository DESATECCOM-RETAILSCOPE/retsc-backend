...
### ⚠ Desconexión estructural con el equipo de anotación (#42, Arthur) — mitigada 2026-08-05, corrección de fondo pendiente

**Confirmado con evidencia de código y datos reales (2026-08-05)**: el equipo de anotación (#42) sube el archivo directo al Blob (`global-shelf-training`) e inserta las filas en `RETSC_AI_TRAINING_PHOTOS`/`RETSC_AI_TRAINING_ANNOTATIONS` directo contra `sqldb-rscope-prod`, **sin pasar por `POST /api/shelf-photos/upload`** (el único endpoint de este backend que inserta en esas tablas — `trainingPhotoRepo.insert()` tiene un solo caller en todo el repo, `shelfPhotoUploadService.js:245`, y `annotationRepo.insert()` no tiene ningún caller). Evidencia: una fila real (`photo_id=1`) apareció con `uploaded_by_enterprise_id=37` (no-NULL — el flujo real siempre lo deja `NULL`) y `photo_notes=NULL` (el flujo real SIEMPRE lo puebla en la Etapa 7) — combinación imposible de producir por `uploadShelfPhoto`.

**Consecuencia**: como el registro en Custom Vision (`cvImageId`, Etapa 6 de `uploadShelfPhoto`) solo ocurre en ese endpoint, **ninguna foto subida por #42 tiene `cvImageId`** — sin eso, `syncRegionsForPhoto` no puede asociar cajitas a ninguna imagen en CV. Mientras #42 no use el endpoint correcto, cada una de sus fotos llega al sync sin poder sincronizarse.

**Mitigación implementada (Opción C, 2026-08-05, NO es la corrección de fondo)**: `annotationSyncService.autoRegisterImage()` — si `syncRegionsForPhoto` no encuentra `cvImageId` en `photo_notes`, en vez de fallar: descarga el blob (`blobStorageService.downloadFromContainer()`, requiere que `blob_path` exista y el blob esté realmente en el container), lo sube a Custom Vision (`createImageFromData()`), y persiste `photo_notes` con el mismo formato que usa `uploadShelfPhoto` (`trainingPhotoRepo.updatePhotoNotes()`, nueva). Deja siempre un `console.warn` explícito (`"entró SIN cvImageId (no pasó por uploadShelfPhoto...) — corrección de fondo pendiente"`) para que quede rastro de cada vez que esto ocurre — nunca es silencioso. Si la foto tampoco tiene `blob_path` (ni siquiera el archivo llegó), falla con un error claro en vez de intentar auto-registrar sin nada que descargar.

**Corrección de fondo pendiente (Opción A, NO implementada)**: que #42 suba las fotos vía `POST /api/shelf-photos/upload` (que ya hace las 8 etapas: calidad, dedup, Blob, registro en CV, insert) en vez de escribir directo a Blob/SQL. Requiere coordinación con el equipo de Arthur, no es un cambio de este backend.

**Hallazgo adicional al probar el fix contra `photo_id=1` real**: el auto-registro funcionó (pobló `photo_notes` con un `cvImageId` real), pero el sync de regiones en sí falló con `400 BadRequestImageRegions: Duplicate image regions` — sus 73 "cajitas" resultaron ser solo **5 posiciones de bbox únicas, cada una repetida** (14, 4, 15, 17 y 16 veces). Es el caso borde ya documentado arriba ("Dos cajitas con coordenadas idénticas..."), pero a una escala mucho mayor de lo que ese comentario anticipaba — probablemente un problema del lado del canvas de #42 (¿doble-submit, o un bug generando cajitas repetidas?), no de este backend. **No se corrigió** (no se deduplicaron anotaciones sin autorización explícita) — `photo_id=1` queda con `cv_sync_status='ERROR'` hasta que el equipo decida qué hacer con esos datos.
