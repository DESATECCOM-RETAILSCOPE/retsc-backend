...
- **Known follow-up, not fixed as part of this pass**: `scripts/test-annotation-review.js` and `scripts/test-model-versioning.js` still assume the pre-split schema (they insert/read the old moved columns directly against the DB) and will need updating before they're trusted again.
