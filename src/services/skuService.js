...
// Validación EAN según spec: numérico + longitud 8, 12 ó 13
function validateEAN(gtin) {
  ...
  if (![8, 12, 13].includes(str.length)) return { valid: false, reason: 'EAN inválido — debe tener 8, 12 o 13 dígitos' };
  return { valid: true };
}
...
function parseSkuExcel(filePath) {
  ...
  if (missing.length > 0) {
    throw svcError(`Archivo inválido o columnas faltantes: ${missing.join(', ')}`, 400);
  }
...
const processSkuExcel = async (filePath, enterpriseCategoryId, enterpriseId) => {
  ...
  if (!entCat) {
    throw svcError('La categoría comercial no existe o no pertenece a este enterprise', 400);
  }

  // detection_category_id: usa la categoría DTC resuelta; si no hay, cae al selected
  const detectionCategoryId = entCat.resolved_category_id ?? entCat.selected_category_id;
